...
class InsightScanner(BaseScanner):
    ...
    def run(self) -> Optional[list]:
        ...
        self.data_provider.set_normal_baseline(
            self.collected_data.get("normal_baseline", None)
        )
        ...
        try:
            ...
            if not raw_res or "error" in raw_res:
                error_msg = (
                    raw_res.get("error", "Unknown AI error")
                    if raw_res
            ...
            indented_ai_report = "\n".join(
                [f"  {line}" for line in ai_analyst_text.split("\n")]
            )
    ...
    def format_tree_visual(
        self, node, prefix="", is_last=True, is_root=True, target_id=None
    ):
        # 1. 트리 마커 설정
        ...
        # 4. 한 줄 구성 (컬럼 간격 최적화)
        line = (
            f"{prefix}{current_marker}{display_name:<30} "
            f"(ID: {node_id:<7}) | "
            f"D: {delta:>7.1f}ms | "
            f"W: {wait:>6.1f}ms | "
        ...
        for i, child in enumerate(children):
            ...
            result += self.format_tree_visual(
                child, new_prefix, last_child, False, target_id
            )
    ...
    def generate_final_report(self, summary_context, ai_res):
        ...
        start_name, end_name = (
            range_str.split(" ~ ") if " ~ " in range_str else (range_str, "")
        )
        ...
        indented_ai_report = "\n".join(
            [f"  {line}" for line in analysis_text.split("\n")]
        )

        # 5. 트리 비주얼 생성
        target_id = ai_res.get("target_id", 0)
        tree_table = self.format_tree_visual(
            summary_context.get("evidence_room_full_tree", {}), target_id=target_id
        )
