...
class ExecutorThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            eng = self.engine
            if eng:
                eng.run(output_callback=cb, model_name=self.model_name, mode=self.mode)
        finally:
            pass
...
class TraceGui(ctk.CTk):
    def __init__(self):
        ...
        ctk.set_default_color_theme(
            "blue"
        )  # Keep blue theme but customize colors below
            ...
            try:
                ...
                try:
                    ...
                    if eng and eng.llm_requester:
                        ...
                        if models:
                            self.after(
                                0, lambda m=models: self.model_combo.configure(values=m)
                            )
