...
class ExecutorThread(threading.Thread):
    ...
    def run(self):
        try:
            if self.engine:
                self.engine.run(mode=self.mode)
        finally:
...
class TraceGui(ctk.CTk):
    def __init__(self):
        ...
        self.engine = Engine(window=self)
    ...
    def _on_selected_incidents_ready(self, incident_list, current_choice=None):
        if (
            hasattr(self, "selected_incidents_spinner")
            and self.selected_incidents_spinner
        ):
    ...
    def _initialize_executor(self):
        ...
        self.engine.start(
            log_callback=self._append_line,
            range_callback=None,
    ...
    def _on_load_data(self):
        ...
        if not api_key and client_type == "google_studio":
            messagebox.showwarning(
                "Missing API Key", "Google Studio requires an API Key."
            )
            return
        ...
        def load_task():
            try:
                # AI 클라이언트 초기화 및 설치된 모델 목록 가져와서 기본값 설정
                try:
                    eng = self.engine
                ...
                self.engine.load(
                    ...
                    api_key=api_key,
                    chart_canvas=self.chart_canvas,
                )
    ...
    def _on_load_success(self):
        ...
        # Disable configuration inputs but keep mode enabled for selection
    ...
    def _on_load_error(self, error):
        ...
        self.realtime_status.configure(
            text=f"❌ ERROR: {str(error)}", text_color="#F85149"
        )
    ...
    def _on_run(self):
        ...
        for widget, title in [
            (self.text_edit, "AI ANALYSIS SESSION"),
            (self.system_edit, "SYSTEM LOG SESSION"),
        ]:
    ...
    def _on_finished(self):
        def finalize():
            ...
            messagebox.showinfo(
                "Done",
                "Analysis finished. Logs have been preserved. You can run another analysis or load new data.",
            )
    ...
    def _on_restart(self):
        if not messagebox.askyesno(
            "Restart",
            "Restart the application? All current session data will be cleared.",
        ):
            return
    ...
    def _build_ui(self):
        ...
        self.api_key_label.grid(row=4, column=0, padx=25, pady=(5, 0), sticky="ew")
        ...
        self.api_key_edit.grid(row=5, column=0, padx=25, pady=(0, 15), sticky="ew")
        ...
        self.btn_load_data.grid(row=6, column=0, padx=25, pady=(0, 20), sticky="ew")
        ...
        self.chart_control_frame = ctk.CTkFrame(
        ...
        self.chart_canvas = ctk.CTkCanvas(
            self.chart_frame, bg="#010409", highlightthickness=0, height=200, bd=0
        )
        ...
        self.left_scrollbar = ctk.CTkScrollbar(
            self.left_pane, command=self.text_edit.yview
        )
        ...
        self.right_scrollbar = ctk.CTkScrollbar(
            self.right_pane, command=self.system_edit.yview
        )
        self.right_scrollbar.pack(side="right", fill="y", padx=(0, 5), pady=5)

        self.system_edit.pack(
