...
class PointScanView:
    ...
    def draw_latency_distribution(self, canvas: ctk.CTkCanvas):
        ...
        if not self.current_incidents:
            canvas.create_text(
                W / 2, H / 2, text="WAITING FOR HOTZONE ANALYSIS...", fill="#4B5563"
            )
            return
        ...
        if self.selected_start_index != -1 and self.selected_end_index != -1:
            hot_segments = self.milestones_registry[
                self.selected_start_index : self.selected_end_index + 1
            ]
        ...
        if len(hot_segments) == 0:
            ...
            margin = (
                self.milestones_registry[0]["ts_s_end"]
                - self.milestones_registry[0]["ts_s_start"]
            )
        ...
        for m in self.milestone_marks:
            if focus_start <= m["ts_s"] <= focus_end:
                ...
                canvas.create_line(
                    mx, PY_TOP - 20, mx, BASE_Y, fill="#30363D", dash=(2, 2)
                )
                canvas.create_text(
                    mx + 5,
                    PY_TOP - 35,
        ...
        canvas.create_text(
            PX + 5,
            BASE_Y + 10,
            text=f"Start: {focus_start:,} ns {start_name} {self.selected_start_index}                                Total Observed Delay: {total_observed_delay_ms:.2f} ms",
            fill="#8B949E",
            anchor="nw",
        )
        canvas.create_text(
            PX + CHART_W - 5,
            BASE_Y + 10,
            text=f"End: {focus_end:,} ns {end_name} {self.selected_end_index}",
            fill="#8B949E",
            anchor="ne",
        )
    ...
    def draw_slice(self, canvas: ctk.CTkCanvas, focus_info):
        ...
        # 구분선 (슬라이스 뷰 상단)
        canvas.create_line(
            10, y_cursor - 10, canvas.winfo_width() - 10, y_cursor - 10, fill="#30363D"
        )
        canvas.create_text(
            PX,
            y_cursor - 5,
            text="THREAD TIMELINE (Shadow: Normal / Box: Slow Actual)",
            fill="#8B949E",
            anchor="nw",
            font=("Segoe UI", 8, "bold"),
        )
